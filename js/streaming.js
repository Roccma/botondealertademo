        ...
        if (llamadaDesconectada == false) {
            ...
            contadorDesconexion = window.setTimeout(function() {
                if (!llamadaFinalizada) {
                    ...
                    jQuery.ajax({
                            url: 'https://botondealerta-services.herokuapp.com/usuarios.php/ActualizarEstadoLlamada?id=' + id + "&cantidad_desconexiones=" + cantidad_desconexiones + "&desconectada=1",
                            type: 'GET',
                            dataType: 'json'
                        })
                        .done(function(response) {
...
socket.on('reconnected', function(data) {
    ...
    if (data['sessionId'] == sessionId) {
        ...
        jQuery.ajax({
                url: 'https://botondealerta-services.herokuapp.com/usuarios.php/ActualizarEstadoLlamada?id=' + id + "&cantidad_desconexiones=" + cantidad_desconexiones + "&desconectada=0",
                type: 'GET',
                dataType: 'json'
            })
            .done(function(response) {
...
socket.on('finish_help', function(data) {
    ...
    $.ajax({
        url: "https://botondealerta-services.herokuapp.com/usuarios.php/FinLlamada?callid=" + id + "&url=&date=&latitud=" + latitud + "&longitud=" + longitud,
        type: 'GET',
        dataType: 'json'
    });
});
